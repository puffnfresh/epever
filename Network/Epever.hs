...
encapsulatedInterfaceTransport =
  flip runModbusRequest EncapsulatedInterfaceTransportRequest
...
rtuReadInputRegistersVolts :: Fold (RunGetResult a RtuResponse) Volts
rtuReadInputRegistersVolts =
  rtuReadInputRegistersWord16 . to volts
...
volts :: Word16 -> Volts
volts a =
  Volts $ (fromIntegral a :: Double) / 100
...
readAddressVolts :: Address -> SerialPort -> IO (Maybe Volts)
readAddressVolts r h =
  let request = ReadInputRegistersRequest (Register r (RegisterCount 1))
  in (^? rtuReadInputRegistersVolts) <$> runModbusRequest h request
...
readAddressWatts r h =
  ...
  in (^? rtuReadInputRegistersWatts) <$> runModbusRequest h request

readAddressWattHours :: Address -> SerialPort -> IO (Maybe WattHours)
readAddressWattHours r h =
  let request = ReadInputRegistersRequest (Register r (RegisterCount 2))
  in (^? rtuReadInputRegistersWattHours) <$> runModbusRequest h request

readAddressPercent :: Address -> SerialPort -> IO (Maybe Percent)
readAddressPercent r h =
  let request = ReadInputRegistersRequest (Register r (RegisterCount 1))
  in (^? rtuReadInputRegistersPercent) <$> runModbusRequest h request

readChargingEquipmentInputVoltage :: SerialPort -> IO (Maybe Volts)
readChargingEquipmentInputVoltage =
  readAddressVolts chargingEquipmentInputVoltage
